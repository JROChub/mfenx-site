...
function assertCommercialUiBoundary() {
  ...
  for (const link of document.querySelectorAll("[data-final-role]")) {
    assert(!isBlockedActiveDistribution(link.dataset.finalRole), "active UI exposes a restricted evidence role");
  }
}
...
async function fetchFinalIndexed(entry, maxBytes = 2 * 1024 * 1024) {
  ...
  const expectedHistoricalUrl = new URL(entry.public_path, "https://mfenx.com/lightsout/validation-record/");
  assert(plannedUrl.href === expectedHistoricalUrl.href, "validation indexed historical path changed: " + entry.role);
...
function validateFinalStatus(status) {
  ...
  assert(status.validation.execution_contract === "complete" && status.validation.reference_verification === "complete" && status.validation.measured_reproduction === "complete", "validation component states changed");
...
async function loadValidationRecordPack() {
  ...
  const pack = { index, byRole, prepublication, selected, scalingCells, record, retrieval };
...
function enableFinalLinks(pack) {
  document.querySelectorAll("[data-final-role]").forEach((link) => {
    ...
    assert(entry, "evidence link names an unindexed final role " + role);
    assert(
      !isBlockedActiveDistribution(role, entry.public_path),
      "active UI attempted to enable a restricted software distribution role"
    );
...
function displayValidationRecord(pack) {
  const recordStates = [
    ...
    ["record-evaluation", "SIGNED / FROZEN", "pass"]
  ];
...
async function loadAndValidate() {
  ...
  try {
    ...
    releaseState("pass", "frozen Validation Record verified");
  } catch (error) {
    ...
    if (publicationPending) {
      ...
      byId("verification-copy").textContent = "The signed Validation Record publication binding is being finalized. Historical evidence links remain disabled until every pinned digest and live-retrieval check is present.";
