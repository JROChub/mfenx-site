...
function renderInspector(packet) {
  ...
  el["summary-tabs"].replaceChildren(...audiences.map((audience) => {
    ...
    const selected = audience === currentAudience;
    button.type = "button";
    button.className = selected ? "active" : "";
    button.id = `summary-tab-${audience.replace(/[^a-z0-9_-]+/gi, "-").toLowerCase()}`;
    button.setAttribute("role", "tab");
    button.setAttribute("aria-controls", "summary-text");
    ...
    button.addEventListener("keydown", (event) => {
      const current = audiences.indexOf(audience);
      let next = null;
      if (event.key === "ArrowRight" || event.key === "ArrowDown") next = (current + 1) % audiences.length;
      if (event.key === "ArrowLeft" || event.key === "ArrowUp") next = (current - 1 + audiences.length) % audiences.length;
      if (event.key === "Home") next = 0;
      if (event.key === "End") next = audiences.length - 1;
      if (next === null) return;
      event.preventDefault();
      currentAudience = audiences[next];
      renderInspector(packet);
      document.getElementById(`summary-tab-${currentAudience.replace(/[^a-z0-9_-]+/gi, "-").toLowerCase()}`)?.focus();
    });
    return button;
  }));
  const summary = packet.summaries?.[currentAudience];
  const activeTab = el["summary-tabs"].querySelector('[aria-selected="true"]');
  if (activeTab) el["summary-text"].setAttribute("aria-labelledby", activeTab.id);
  else el["summary-text"].removeAttribute("aria-labelledby");
